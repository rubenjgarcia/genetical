...
function Genetical(options) {
    ...
    this.evolve = function () {
        ...
        if (this.selectionStrategy !== Genetical.ROULETTEWHEELSELECTION && this.selectionStrategy !== Genetical.STOCHASTICUNIVERSALSAMPLING) {
            throw "'selectionStrategy' must be Genetical.ROULETTEWHEELSELECTION or Genetical.STOCHASTICUNIVERSALSAMPLING"
        }

        if (this.selectionStrategy === Genetical.ROULETTEWHEELSELECTION) {
    ...
    this.stochasticUniversalSampling = function () {
        var aggregateFitness = 0;
        for (var i = 1; i<this.population.length; i++) {
            aggregateFitness += adjustScore(this.population[i].score);
        }
        ...
        for (var i = 1; i<this.population.length; i++) {
            cumulativeExpectation += adjustScore(this.population[i].score) / aggregateFitness * selectionSize;
